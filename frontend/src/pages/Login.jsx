...
const Login = () => {
  const { handleChange, handleSubmit, loading } = useContext(storeContext)
  return (
      ...
      <div className='w-[500px] sm:mx-5 border-[#dddddd] border-[1px] p-5 flex flex-col gap'>
        ...
        <form className='flex flex-col gap-2' onSubmit={handleSubmit}>
          <div className='flex flex-col gap-1'>
            ...
            <input onChange={handleChange} className='border-[1px] focus:border-[#28a745] outline-none px-5 border-[#dddddd] h-[52px]' type="email" placeholder="Enter your email" name='email' required />
          </div>
          <div className='flex flex-col gap-1'>
            <label className='lg:text-[16px] text-[#4f5050] sm:text-[14px]'>Password</label>
            <input onChange={handleChange} className='border-[1px] focus:border-[#28a745] outline-none px-5 border-[#dddddd] h-[52px]' name='password' type="password" placeholder="Enter your password" required />
          </div>
